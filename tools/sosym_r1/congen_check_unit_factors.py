...
Cowork-layer analysis harness for SoSyM checklist items **C10** (the R1-Q4 cost
accounting) and **B17** (the positive-example pruning in Algorithm 2). Reads only
...
the phase is simply absent from this data. Closing that is checklist item
C10(a). Until then, treat ``acq_calls`` as "AcqMss only" rather than
"everything except Reduce".

STALENESS
---------
``data/results/congen/`` predates ADR-0015 (unseeded example-mode pool),
ADR-0016 (QuAcq bias order), ADR-0017 (Reduce dedup order) and checklist items
C6, C7, C11. The **KB contents** these runs produced are therefore not the
...
def render(agg: list[dict], rows: list[dict], prov: list[dict],
    ...
    add("⚠ **Stale inputs.** These results predate ADR-0015, ADR-0016, ADR-0017 "
        "and checklist items C6, C7, C11, so the knowledge bases they produced "
    ...
    add("⚠ **Preprocessing is absent.** ConGen calls GenerateNE with "
        ...
        "`acq_calls` is AcqMss only. Closing that gap is checklist item C10(a).")
    add("")
    ...
    if tested:
        add(f"  On the {tested} knowledge base(s) where all three are present, "
            ...
            "B17's pruning **per knowledge base** rather than as a single "
