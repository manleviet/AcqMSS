...
class ConGenModel:
    ...
    def prepare(
        ...
        # set_kb = oracle.get_kb() + self._task.set_kb  # merge two kbs and assumptions without skipping
        # assumptions = oracle.get_assumptions() + self._task.assumptions
        #
        # # prepare negative test cases
        # testsuite = self._task_input.negative_test_cases
        # if testsuite is None or len(testsuite.testcases) == 0:
        #     # No negative test cases, so no NE to generate; return prepared task as is
        #     return self._task
        #
        # # for testcase in testsuite.testcases:
        # #     id_assumption = self.next_tseitin_var
        # #     desc_parts = []
        # #     literals = []
        # #
        # #     for assignment in testcase.assignments:
        # #         if assignment.feature not in self.variables:
        ...
        # #         var = self.variables[assignment.feature] if assignment.value else -self.variables[assignment.feature]
        ...
        # #         var = self.variables[assignment.feature] if assignment.value else -self.variables[assignment.feature]
        # #         literals.append(var)
        # #         set_kb.append([var, -id_assumption])
        # #
        # #     assumptions.append(id_assumption)
        ...
        #
        # generate_ne = GenerateNE(checker, oracle)
