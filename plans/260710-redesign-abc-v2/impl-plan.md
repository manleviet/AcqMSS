...
- **11.3:** oracle dùng `prepare_variable_assignments` chung (từ T5). **+⑤:** `QuAcqModel:124` có BẢN SAO wrapper `_config_to_assumptions` (cùng anti-pattern dựng `AssignmentAssumptionMap` mỗi query) → dọn ở đây, model dựng map 1 lần & phơi `assignment_map` (như FMOracleModel đã làm ở 11.2).
- **11.4 ⬆️ NÂNG CẤP (ADR-0009 — tách vai):** `conacq/oracle/oracle_data.py` (mới) — **`OracleData`** frozen snapshot fold lên model lúc `build()`, implement **`BGProvider` + `KBProvider`** (= composite `PreparationOracle` của T11.1 — ĐÓ chính là hình dạng OracleData; đừng xoá). Job ② (`get_bg_data`/`get_root_clauses`/`get_kb`/`get_assumptions`/`get_c`) rời KHỎI oracle sang OracleData; `GenerateNE` + 2 task_prep + builders nhận OracleData, KHÔNG nhận oracle sống. A6 tan **theo cấu trúc** (oracle không sở hữu thứ nó từng làm nhiễm). `ConGenModel.prepare_task` bỏ tham số oracle; `QuAcqModel.prepare_task(task_input=None)` từ chối non-empty; [#3] `FMOracleModel.prepare_task` fail-fast `assert task_input is None`; [#1] `bg_data` **eager trong `build()`**, prepare_task pure (bỏ cache + RuntimeError thứ-tự-gọi).
  ...
  - **⑥ công thức T3 cho cả 3 model** — state mỗi model đang ôm (đo): DiagnosisModel 1 (xong) · QuAcqModel 7 · FMOracleModel 14 · **ConGenModel 21** ⚠️. Công thức: tách builder → model bất biến → `prepare_task(task_input) -> PreparedTask` thuần → `use_incremental` thành tham số operation.
  - **+nợ T6 (bỏ sót):** FMOracleModel là model DUY NHẤT còn ôm method builder (`from_fm`/`build`/`set_incremental`) — T6 tách builder cho ConGen/QuAcq nhưng trượt oracle → `FMOracleModelBuilder` kế thừa `AbstractModelBuilder`.
...
- **11.5 [#10]:** `complete_configuration` giữ MỘT solver bền; per-call chỉ solve dưới assumptions. Safety-net 11.0 canh kết quả. **+④ tối ưu delta (CỔNG KÉP):** trên đường oracle base (assumption ràng buộc FM) luôn bật/không đổi ⇒ `delta = (toàn bộ assignment-assumption) \ cfg` ⇒ bỏ cả phép nối base lẫn quét `O(|assumptions|)`. 🚨 NHƯNG `_compute_delta` là của `CheckerBase` — MỌI checker dùng chung; với FastDiag/QuickXPlain/HSDAG `set_c` LÀ không gian tìm kiếm đang thu hẹp (đổi mỗi call) ⇒ áp vào `_compute_delta` chung sẽ làm SAI chúng, âm thầm. **Cổng:** (a) chứng minh chỉ chạm đường oracle, KHÔNG chạm `CheckerBase._compute_delta` chung; (b) ĐO TRƯỚC (rule #7).
...
**✅ 11.2 thực thi (sửa A6 — triệu chứng; class-fix ở T11.4):**
- **Refutable Q (bác bỏ được):** oracle KHÔNG BAO GIỜ dựng model với `configuration != None` (`FMOracleModel.configuration` = None ở init, `build()` truyền None, 0 assignment `.configuration =`) ⇒ `task.set_c` LUÔN == FM-only slice ⇒ `get_c()` == `_base_set_c` cũ ⇒ xoá an toàn, hành vi y hệt.
- `is_valid` → `self._oracle_model.config_set_c(assignments)` (thuần: `get_c() + config_to_assumptions(query)`, KHÔNG rebind). Xoá `with_configuration` + `_base_set_c` + import `replace` chết. `prepare` tính `set_c` local (giữ nhánh prep-config hợp lệ).
- **3 xfail → PASS (đúng 3, không hơn):** A6 get_c-invariant · with_configuration-gone · base_set_c=0 → chuyển sang assert thường (guard vĩnh viễn). 5 xfail còn lại giữ nguyên (T11.4×4, T11.5×1).
- **Test sửa (liệt kê):** `test_oracle_model.py` ×3 (49/68/78 → pin hợp đồng mới config_set_c + no-mutation) + `test_assumption_slicer.py` Site 5 (`_base_set_c` → `get_c()`, cùng giá trị [4,6,8]; impl-plan cho phép "+Site 5").
...
**T11.4b — T3 recipe × 3 model (chia 3 commit: 4b1 FMOracle → 4b2 QuAcq → 4b3 ConGen; Cowork chốt "stateless helpers on model" cho ripple QuAcq).**
- **4b1 (FMOracleModel → pure KB):** xoá `_task/_description_provider/_bg_data/assignment_map/_use_incremental` + property + `set_incremental` + 3 RuntimeError + `get_kb/get_assumptions/get_c`; `prepare()`→`prepare_task(self,task_input)->PreparedTask` thuần; `build()` KHÔNG auto-prepare. **+2 sửa sau review (fix-before-commit):** ① xoá `get_variables/get_variable_ids/get_fm_clauses` khỏi model (2 accessor cho 1 sự thật — pattern _pos/_neg/_base_set_c); FMOracle hiện thực CatalogProvider (derive từ `model.name_to_id`) + `_fm_clauses()` derive từ `constraint_map`. ② `_build_oracle_data` RA KHỎI FMOracle → `FMOracleTaskPreparation.prepare(model)->OracleData` (oracle nhận, không tự dựng job②; ADR-0009); checker dựng từ snapshot (build_checker chỉ đọc set_kb+assumptions). root_clauses derive `next(iter(constraint_map))` (root = key đầu, cùng bất-biến bg_data). **Model = 5 thành viên** (`__init__·name_to_id/id_to_name kế thừa·from_fm·build·prepare_task`). Guard **holds_no_provisioning_object lật ở 4b1** (không phải 4b3 — nó duyệt vars(oracle), oracle chỉ giữ _oracle_model; sửa lỗi kỳ vọng Cowork). Gate: 466 pass · 4 xfail · 13 char XANH · fixtures RỖNG.
- **4b2/4b3:** QuAcqModel/ConGenModel theo cùng công thức; QuAcq cần thread PreparedTask vào FindC/DiscrimGen/QueryProvider (helper stateless nhận task/assignment_map). `prepare_task_is_unified` + `no_call_prepare_first_runtime_error` lật ở 4b3 (model cuối). Cắt vòng `use_incremental` runner↔model: xoá `OracleBiasModelBuilder.use_incremental()` vestigial + `.use_incremental(...)` ở congen_runner:81/quacq_runner:104 khi runner đọc `self.use_incremental` (thêm `self.use_incremental` vào BaseRunner).

**📌 T11.4c — GHI (chưa làm):**
- **API DIET:** 4 method public FMOracle ZERO production-consumer (`get_root_feature·get_cnf_clauses·get_num_constraints·get_next_available_id` → chỉ `tests/t11_oracle_net_helpers.py`; `get_fm_data` → chỉ `tests/test_quacq.py`). Sống chỉ vì lưới T11.0 ghi lại (siêu-tập 14 method cho an toàn) ⇒ lưới đang giữ API CHẾT. Gom dữ-liệu-tham-chiếu-FM vào FMData frozen, xoá getter.
- **🔒 TINH CHỈNH LUẬT GOLDEN (chốt — ca đầu tiên):** *được BỎ một KEY của golden khi API nó ghi lại bị XOÁ CÓ CHỦ ĐÍCH; KHÔNG BAO GIỜ ĐỔI một GIÁ TRỊ. Việc bỏ key phải là MỤC ĐÍCH của commit đó, review đúng như vậy — không bao giờ là tác dụng phụ.* Vết nứt DUY NHẤT được phép trong luật "không regenerate golden".
- **`FMOracleModelBuilder`** (nợ T6 — model cuối còn tự dựng qua from_fm/build) inheriting AbstractModelBuilder.
- **Lưu tên root tường minh lúc build** (sửa triệt để dev-2): 4b1 derive root_clauses = `constraint_map[next(iter(...))]` (root = key đầu, đã pin bằng `test_root_constraint_is_the_first_constraint_map_key` trên 3 FM). Khi FMOracleModelBuilder ra đời, `build()` (có flamapy fm) lưu `root_feature = fm.root.name` tường minh ⇒ bỏ quy-ước "key đầu".
- **Gom FM impl vào `conacq/oracle/fm/`** + tách 3 class của `fm_oracle_model.py` (FMOracleModel · FMOracleTaskPreparation, + bất kỳ result type) thành module riêng. Làm ở 4c (file đã bị viết lại cho builder extraction) — ĐỪNG move file giữa arc (churn import path lưới khi 13 golden phải xanh).
- **📌 T11.5 (#10):** `FMOracle._fm_clauses()` dựng lại clause-list MỖI completion (hot-path) → persistent completion solver, cùng #10.
