...
**4. `data/bias-config/README.md` was stale. Fixed.** It logged "Cross-tree mode: all"
for REAL-FM-4, fqa and arcade-game; their YAML says `extracted`, and the built biases
agree with the YAML — the difference between pairing every feature and pairing 34, 14
and 34. It was a captured console log, so it described the run that made it rather than
the files it names, and nothing in the repository disagreed with it.

It is now rendered by `apps/describe_bias_configs.py` from the YAMLs: one row per config
with features, leaves, binary/group candidates, mode, k, and the `|B|` the closed form
implies — which agrees with every committed `-bias-stats.txt`, linux-2.6.33.3 at
13,000,103 included. Two checks hold it there, and in both the check IS the render, so
it cannot drift from what it checks:

| where | when |
|---|---|
| `tests/test_bias_io.py::test_bias_config_readme_is_the_render_of_the_configs` | every commit |
| `scripts/check-release-hygiene.sh` (now six checks) | before a carve is published |

Both were shown red first, on the historical drift itself — fqa's mode flipped back to
`all` — and the gate was falsified against a carved tree, exit 1, then 0 after
re-rendering.
...
Carve `0f51bfe` from source `29ef06e`, published to `main` and `v1.0.0` (verified by
fetching the tag back: both resolve to `0f51bfe`). `CITATION.cff` `date-released:
2026-09-20`, the tag day. It supersedes carve `88667be`, published an hour earlier from
`cadc596`, which is identical except for finding 4.
...
| `pytest tests/ -q` | **342 passed**, 0 failed, 0 skipped — no red test names |
| hygiene scan | six checks, all pass (carve step 5) |

Here: **681 passed, 1 skipped**, baseline moved at `fff6a48` and recorded in `CLAUDE.md`
with the commit and the environment, as that file requires.
...
3. ~~`data/bias-config/README.md` (finding 4)~~ — resolved; see finding 4.
