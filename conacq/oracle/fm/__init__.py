...
``FMOracleModelBuilder``, the pure ``FMOracleTaskPreparation`` (job ②), and the
``FMData`` metadata snapshot. Kept apart from the FM-agnostic pieces in
``conacq.oracle`` — the role protocols, the frozen values (``OracleData``,
``BGData``), and the other oracles (``CachedOracle``, ``UserPromptOracle``).

This ``__init__`` re-exports the five classes for convenience; the parent
