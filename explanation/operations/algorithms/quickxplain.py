...
class QuickXPlain:
    ...
    def __init__(self, checker: ConsistencyChecker, profiler_instance: AbstractProfiler =None) -> None:
        """
        Initialize QuickXPlain algorithm.

        :param checker: ConsistencyChecker instance
        :param profiler_instance: Optional profiler instance. If None, uses global default profiler.
    ...
    @count_calls('qx_calls')
    @measure_time('qx_runtime')
    def _qx(self, set_d: List, set_c: List, set_b: List) -> List:
        ...
        if len(set_d) != 0 and not self.checker.is_consistent(set_b):
            logging.debug('<<< return Φ')
            return []
