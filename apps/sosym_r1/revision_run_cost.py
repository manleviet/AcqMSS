...
cannot see a number a sentence repeats, and the revision added three kinds:
  ...
  * S5.5.1's read-aloud row. It walks the reader through KB1 under RS(n) --
    ...
    complexity bound of S5.1 at that row, with gamma = 203 constraints removed and
  ...
  * S5.3's justification for the two n/a cells. KB5's RS(n) fold takes 4.1-4.3 h
  ...
  * S5.4's identity between two sampling strategies: "The number m of RS(m) is the
...
# S5.5.1, the row the paper reads aloud, and the sentence around it.
READ_ALOUD = {
