...
**Critical Implementation Detail (Feature ID Consistency)**:
The Oracle's `_build_feature_ids()` method uses flamapy's variable mapping (tree traversal order) as the authoritative source of truth. This ensures that feature_ids match the SAT variable IDs in CNF clauses generated by `FmToPysat`. Using alphabetical sorting would cause a critical mismatch, leading to incorrect Oracle validation results.
