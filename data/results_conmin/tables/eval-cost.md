...
Learning cost. \textsc{ConMin}(raw,$k{=}1$) sizes, checks and time, with \textsc{QuAcq} time and queries. budget/$|B|$ = \texttt{max\_queries}$/|B|$, the query budget fixed before the run (busybox $0.75<1$, below $|B|$), independent of the stop reason. On a wall-clock timeout, t(s) is the timeout wall and queries the count reached, but the budget was still \texttt{max\_queries}. Exclude-2COV means.

| KB | $|A|$ | $|C|$ | $supp$ | $|U|$ | checks | t(s) | QuAcq-ex t(s) | q | QuAcq t(s) | q | budget/$|B|$ |
|---|---|---|---|---|---|---|---|---|---|---|---|
...
| $KB_{5}$ | 3,048 | 1.8 | 1,360 | 0.0 | 10,676 | 59,905.0 | 3,568.4 | 296.3 | 36,754.4† | 1,901.0† | 0.75 |
